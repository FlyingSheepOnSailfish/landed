...
var __Json = function() {
    ...
    return {
        getDatabase: function() {
            return JSDB.JSONStorage.openDatabase(JSDB.JSONStorage.Masterdata);
        },
...
//TODO: Consider adding a further level "Reference Point" as a child of site,
//      so Porterville could be a site, and the various take offs and landings would be reference points
//      This might help where an area has many sites, landings take offs etc.

        //each Site belongs to an area
        //e.g. the sites "Porterville Dasklip" and "Porterville Landing" belong to the area "South Africa"
        getSites: function(area_id) {
            var db = this.getDatabase();
            //('SELECT * FROM Site t where t.area_id= ?;', [area_id]);
            var query = "$.landed.area[?(@.id == " + area_id + ")].site[*]"
            var rs = jsonSupport.executeQuery(db, query);
            return rs;
        },
